...
def load_texture(image_path, directory_list):
    if not image_path:
        return None

    image_path = image_path.replace('\\', os.path.sep).replace('/', os.path.sep)
    blender_image = None
    ...
    alt_extensions = ['.dds', '.png', '.tga', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff']
    
    for alt in alt_extensions:
        if alt.lower() != ext.lower():
             candidates.append(base + alt)
             candidates.append(base + alt.upper())

    def try_load(p):
        if os.path.isfile(p):
            try:
                return image_utils.load_image(p, check_existing=True)
            except:
                return None
        return None
    ...
    for cand in candidates:
        if os.path.isabs(cand):
             relative_candidates.append(os.path.basename(cand))
        else:
             relative_candidates.append(cand)
             relative_candidates.append(os.path.basename(cand))

    unique_rel_candidates = []
    seen = set()
    for c in relative_candidates:
        if c not in seen:
            unique_rel_candidates.append(c)
            seen.add(c)

    for directory in directory_list:
        for rel_cand in unique_rel_candidates:
             clean_cand = rel_cand.lstrip(os.path.sep)
             target_path = os.path.join(directory, clean_cand)
             blender_image = try_load(target_path)
             if blender_image:
                 return blender_image

    if not blender_image:
        print(f"Failed to load a texture {image_path}. Checked directories: {directory_list}")
        try:
             blender_image = image_utils.load_image(image_path, place_holder=True, check_existing=True)
        except:
             pass
