...
class CascHandler:
    def __init__(self, storage_path="/Applications/StarCraft II"):
        self.storage_path = storage_path
        self.lib_path = os.path.abspath("CascLib/build/casc.framework/casc")
        self.casc = None
        self.hStorage = None
        self.is_open = False
    ...
    def search_files(self, pattern="*"):
        ...
        if hFind and hFind != -1: # Check for valid handle (some APIs return -1/INVALID_HANDLE_VALUE)
             # Process first file
    ...
    def read_file_content(self, casc_path):
        """Reads file content into memory (bytes) without saving to disk."""
        if not self.is_open:
            return None
