...
class Importer:
    ...
    def create_materials(self):
        ...
        for m3_matref in self.m3[self.m3_model.material_references]:
            ...
            bl_mat = bpy.data.materials.new(name=matref.name)
            bl_mat.use_nodes = True
            bsdf = bl_mat.node_tree.nodes.get('Principled BSDF')
            self.bl_materials[len(ob.m3_materialrefs) - 1] = bl_mat
            ...
            for layer_name in shared.material_type_to_layers[m3_matref.type]:
                ...
                image = shared.load_texture(m3_layer_bitmap_str, search_dirs)
                if image:
                    layer.color_bitmap = image.filepath

                    if bl_mat and bsdf:
                        tex_node = bl_mat.node_tree.nodes.new('ShaderNodeTexImage')
                        tex_node.image = image
                        tex_node.label = layer_name
                        if layer_name in ['norm', 'spec', 'gloss', 'rough', 'met', 'height', 'ao']:
                            try: tex_node.image.colorspace_settings.name = 'Non-Color'
                            except: pass
                        
                        uv_idx = m3_layer.uv_source if 'uv_source' in dir(m3_layer) else 0
                        # Handle UV Source mapping (0=UVMap, 1=UVMap.001, etc.)
                        # Some types like 9 (Particle) or 6 (Planar) might need special handling, 
                        # but for now we map indices directly.
                        
                        channel_settings = m3_layer.color_channels if 'color_channels' in dir(m3_layer) else 0
                        created_layer_nodes[layer_name] = {
                            'node': tex_node, 
                            'uv_idx': uv_idx,
    ...
    def create_mesh(self):
        ...
        for region_ii, region in enumerate(self.m3[self.m3_division.regions]):
            ...
            for batch in region_batches:
                ...
                mat_idx = self.matref_index(batch.material_reference_index)
                bl_mat = self.bl_materials.get(mat_idx)
                if bl_mat and not mesh_ob.data.materials.get(bl_mat.name):
                    mesh_ob.data.materials.append(bl_mat)
